...
+ 可以使用set()进行集合的定义和转换
+ 使用集合推导式完成集合的定义
注意：集合中的元素不能重复，集合中存放的数据：Number,Strings,Tuple,冰冻集合  #冰冻集合/固定集合：一旦创建就不可以进行修改的集合。一种数据类型。
'''
...
# 向集合中追加元素 add()          # append() 向列表的尾部追加新的元素
res = vars.add('def')          # varlist.append('川哥')

# 删除集合中的元素 随机删除一个元素并返回  abc False True 3.1415
# r1 = vars.pop()          #随机删除的剩下的vars内容不一样
...
res = vars.update({1,2,3,4,5})    #{False, 'abc', True, 3.1415, 2, 3, 4, 5, (1, 2, 3), 'love', '123', 123}
#因为1和True只能有一个所以1不显示了
...
# 冰冻集合的推导式
res = frozenset({i<<1 for i in range(6)})   #二进制的数向左移动一位就相当于乘于二
...
varset = {1,2,3,4,5}
# 1.0.普通推导式
newset = {i<<1 for i in varset } #{2, 4, 6, 8, 10}
print(newset)
